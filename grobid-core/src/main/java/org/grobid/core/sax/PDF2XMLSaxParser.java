...
/**
 * SAX parser for XML representation of PDF files obtained via xpdf pdf2xml.
 * All typographical and layout information are defined token by token
 *
...
public class PDF2XMLSaxParser extends DefaultHandler {
    ...
    public PDF2XMLSaxParser(Document d,
                             ArrayList<String> im) {
        doc = d;
        blabla = new StringBuffer();
        images = im;
    ...
    public String getText() {
        ...
        res = res.replace("\u00A0", " "); // stdandard NO-BREAK SPACE are viewed as space
        return res.trim();
    }
    ...
    private static void removeLastCharacterIfPresent(LayoutToken token) {
        if (token.getText() != null && token.getText().length() > 1) {
            token.setText(token.getText()
                    .substring(0, token.getText().length() - 1));
        } else {
            token.setText("");
        }
    }
    ...
    private String modifyCharacter(Character baseChar, Character modifierChar) {
        ...
        if (result == null) {
            System.err.println("FIXME: cannot apply modifier '" + modifierChar + "' to character '" + baseChar + "'");
        }
    ...
    public void endElement(java.lang.String uri,
        ...
        //if (!qName.equals("TOKEN") && !qName.equals("BLOCK") && !qName.equals("TEXT"))
        ...
        if (qName.equals("TEXT")) {
            ...
            if (tok0.length() > 0) {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    if (tok.length() > 0) {
                        ...
                        if ((previousToken != null) && (tok != null) && (previousToken.length() > 0)
                            ...
                            if (leftClass != ModifierClass.NOT_A_MODIFIER || rightClass != ModifierClass.NOT_A_MODIFIER) {
                                ...
                                if (leftClass != ModifierClass.NOT_A_MODIFIER) {
                                    if (rightClass != ModifierClass.NOT_A_MODIFIER) {
                                        assert false;
                                        // keeping characters, but setting class to not a modifier
                                        baseChar = leftChar;
                                ...
                                diaresis = (modifierClass == ModifierClass.DIAERESIS || modifierClass == ModifierClass.NORDIC_RING
                                        || modifierClass == ModifierClass.CZECH_CARON || modifierClass == ModifierClass.TILDE
                                        || modifierClass == ModifierClass.CEDILLA);

                                accent = (modifierClass == ModifierClass.ACUTE_ACCENT || modifierClass == ModifierClass.CIRCUMFLEX
                                        || modifierClass == ModifierClass.GRAVE_ACCENT);

                                if (rightClass != ModifierClass.NOT_A_MODIFIER) {
                                    tok = ""; // resetting current token as it is a single-item
                                }
                            }
                        ...
                        /*StringTokenizer st0 = new StringTokenizer(tok0, TextUtilities.fullPunctuations, true);
                        ...
                        /*boolean punct1 = false;
                              boolean punct2 = false;
                                  boolean punct3 = false;
                                  String content = null;
                              int i = 0;
                              for(; i<TextUtilities.punctuations.length(); i++) {
                                  if (tok.length() > 0) {
                                      if (tok.charAt(tok.length()-1) == TextUtilities.punctuations.charAt(i)) {
                                          ...
                                          if (tok.length() > 1) {
                                              int j = 0;
                                              for(; j<TextUtilities.punctuations.length(); j++) {
                                                  if (tok.charAt(tok.length()-2) == TextUtilities.punctuations.charAt(j)) {
                                                      punct3 = true;
                                                      content = tok.substring(0, tok.length()-2);
                                                  }
                                              }
                                          }
                                      break;
                              ...
                              if (tok.length() > 0) {
                                  if ( (tok.startsWith("(")) && (tok.length() > 1) ) {
                                      if ((punct3) && (tok.length() > 2))
                                          content = tok.substring(1, tok.length()-2);
                                      else if (punct1)
                        ...
                        /*if (punct2) {
                                  ...
                                  if (punct3) {
                                      token = new LayoutToken();
                                      token.setText(""+tok.charAt(tok.length()-2));
                                      if (currentFont != null)
                                              token.setFont(currentFont.toLowerCase());
                                          else
                                              token.setFont("default");
            ...
            // page marker are usefull to detect headers (same first line(s) appearing on each page)
            if (block != null) {
                blabla.append("\n");
                tokenizations.add("\n");
        ...
        /*else if (qName.equals("VECTORIALIMAGES")) {
            if (block != null) {
        		blabla.append("\n");
            	block.setText(blabla.toString());
            	block.setNbTokens(nbTokens);
