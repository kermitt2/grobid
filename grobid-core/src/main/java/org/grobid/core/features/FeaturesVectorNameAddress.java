 ...
 *
 */
public class FeaturesVectorNameAddress {
     ...
     */
    static public String addFeaturesNameAddress(List<LayoutToken> tokens,
                                                List<String> labels,
                                                List<OffsetPosition> placePositions,
                                                List<OffsetPosition> countryPositions,
                                                List<OffsetPosition> titlePositions,
                                                List<OffsetPosition> suffixPositions) throws Exception {
        /*if (locationPlaces == null || locationPlaces.size() == 0 || titlePositions.size() == 0 || suffixPositions.size() == 0) {
        ...
        for(int n=0; n<tokens.size(); n++) {
            ...
            // check the position of matches for name title 
            skipTest = false;
            if (titlePositions != null) {
                if ((titlePositions != null) && (titlePositions.size() > 0)) {
                    if (currentTitlePosition == titlePositions.size() - 1) {
                        if (titlePositions.get(currentTitlePosition).end < n) {
                            skipTest = true;
                        }
                    }
                    if (!skipTest) {
                        for (int i = currentTitlePosition; i < titlePositions.size(); i++) {
            ...
            // check the position of matches for name suffix
            skipTest = false;
            if (suffixPositions != null) {
                if (currentSuffixPosition == suffixPositions.size() - 1) {
                    if (suffixPositions.get(currentSuffixPosition).end < n) {
                        skipTest = true;
                    }
                }
                if (!skipTest) {
                    for (int i = currentSuffixPosition; i < suffixPositions.size(); i++) {
