...
public class XMLWriter {
	 ...
	 */
	public void addStartElement(final String pTagName)
			throws XMLStreamException {
		final XMLEvent tmpEvent = eventFactory.createStartElement(StringUtils.EMPTY,
				StringUtils.EMPTY, pTagName);
		writer.add(tmpEvent);
		writer.flush();
	}
	 ...
	 */
	public void addStartElement(final String pTagName,
			final Iterator<Attribute> pAttributes) throws XMLStreamException {
		final XMLEvent tmpEvent = eventFactory.createStartElement(
				new QName(pTagName), pAttributes, null);
		writer.add(tmpEvent);
		writer.flush();
	}
	 ...
	 */
	public void addStartElement(final String pTagName,
			final Map<String, String> pAttributes) throws XMLStreamException {
		XMLEvent tmpEvent = eventFactory.createStartElement(StringUtils.EMPTY,
				StringUtils.EMPTY, pTagName);
		writer.add(tmpEvent);
		...
		if (pAttributes != null) {
			...
			for (final Entry<String, String> currAttr : attributes) {
				tmpEvent = eventFactory.createAttribute(currAttr.getKey(),
						currAttr.getValue());
				writer.add(tmpEvent);
			}
		}
	 ...
	 */
	public void addStartElement(final String pTagName,
			final String... pAttributes) throws XMLStreamException {
		XMLEvent tmpEvent = eventFactory.createStartElement(StringUtils.EMPTY,
				StringUtils.EMPTY, pTagName);
		writer.add(tmpEvent);
	...
	public void addEndElement(final String pTagName) throws XMLStreamException {
		final XMLEvent tmpEvent = eventFactory.createEndElement(StringUtils.EMPTY,
				StringUtils.EMPTY, pTagName);
		writer.add(tmpEvent);
		writer.flush();
	}
	...
	public static String formatXML(final String pToFormat) {
		...
		try {
			...
			transformer.transform(
					new StreamSource(new StringReader(pToFormat)),
					new StreamResult(indented));
	...
	protected void init() {
		...
		try {
			...
			throw new GrobidException(
					"An error occured while creating the Stax event readers: "
							+ xmlStrExp.toString());
		} catch (FactoryConfigurationError factCongExp) {
			throw new GrobidException(
					"An error occured while creating the Stax event readers: "
							+ factCongExp.toString());
		}
	}
