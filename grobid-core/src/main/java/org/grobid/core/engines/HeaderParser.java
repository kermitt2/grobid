...
public class HeaderParser extends AbstractParser {
    ...
    public String processing(String input, boolean consolidate, BiblioItem resHeader, int startPage, int endPage) {
...
//            int endPage = 2;
            ...
            // input is the pdf file, tmpPath is the tmp directory for the lxml file,
...
//			e.printStackTrace();
            throw new GrobidException("An exception occurred while running Grobid with tmpPath " + pathXML, e);
        } finally {
            doc.cleanLxmlFile(pathXML, true);
        }
    }

    public String processingHeaderBlock(boolean consolidate,
                                        Document doc,
        ...
        try {
            ...
            if (resHeader != null) {
                ...
                if (resHeader.getTitle() != null) {
                    //String temp = utilities.dehyphenizeHard(resHeader.getTitle());
				...
				resHeader.setFullAffiliations(affiliationAddressParser.processReflow(res.toString(),
																					 tokenizations));
						...
						if (authorSegments != null) {
							if (resHeader.getFullAffiliations().size() == authorSegments.length) {
								...
								for (Person pers : resHeader.getFullAuthors()) {
									pers.addAffiliation(
										resHeader.getFullAffiliations().get(authorsBlocks.get(k)));
									k++;
								}
								attached = true;
            ...
            if (resHeader != null) {
                if (resHeader.getPublicationDate() != null) {
                    ...
                    // most basic heuristic, we take the first date - to be revised...
                    if (dates != null) {
                        if (dates.size() > 0) {
    ...
    public void createTrainingHeader(String inputFile,
        ...
        try {
            ...
            pathXML = doc.pdf2xml(true, false, startPage, endPage, inputFile, tmpPath.getAbsolutePath(), false); //with timeout,
            //no force pdf reloading
            ...
            StringBuffer bufferAffiliation = affiliationAddressParser.trainingExtraction(rese,
                    tokenizations);
    ...
    /**
     * Extract results from a labelled header. If boolean intro is true, the extraction is stopped
     * at the first "intro" tag identified (this tag marks the begining of the description).
     * @param result result
...
//                        newLine = true;
            ...
            if ((s1.equals("<title>")) || (s1.equals("I-<title>"))) {
                ...
                // we should check, before adding a new date segment, if it is not already present
...
//                    localFeatures.add(s);
    ...
    private void testClosingTag(StringBuilder buffer,
                                String currentTag0,
                                String lastTag0) {
    ...
    private boolean writeField(StringBuilder buffer,
                               String s1,
                               String lastTag0,
                               String s2,
                               String field,
