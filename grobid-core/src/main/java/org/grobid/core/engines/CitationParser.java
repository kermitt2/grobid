...
public class CitationParser extends AbstractParser {
    ...
    public BiblioItem processing(String input,
                                 boolean consolidate) {
        ...
        try {
            ...
            String ress = FeaturesVectorCitation.addFeaturesCitation(citationBlocks,
                    journalsPositions,
            ...
            if (resCitation != null) {
                ...
                resCitation.setFullAuthors(authorParser.processingCitation(auts));
                if (resCitation.getPublicationDate() != null) {
                    if (dateParser == null) {
                        dateParser = new DateParser();
                    }
                    ArrayList<Date> dates = dateParser.processing(resCitation.getPublicationDate());
                    if (dates != null) {
                        Date bestDate = null;
                        if (dates.size() > 0) {
    ...
    public BiblioItem resultExtraction(String result,
        ...
        int p = 0;
        // iterator for the tokenizations for restauring the original tokenization with
        // respect to spaces
        ...
        while (st.hasMoreTokens()) {
            ...
            if ((s1.equals("<title>")) || (s1.equals("I-<title>"))) {
            ...
            } else if ((s1.equals("<booktitle>")) || (s1.equals("I-<booktitle>"))) {
            ...
            } else if ((s1.equals("<publisher>")) || (s1.equals("I-<publisher>"))) {
                ...
                if (biblio.getVolumeBlock() != null) {
                    if (addSpace)
                        biblio.setVolumeBlock(biblio.getVolumeBlock() + " " + s2, volumePostProcess);
                    else
                        biblio.setVolumeBlock(biblio.getVolumeBlock() + s2, volumePostProcess);
                } else
    ...
    public BiblioItem consolidateCitation(BiblioItem resCitation) {
        try {
            ...
            boolean valid = consolidator.consolidateCrossrefGet(resCitation, bibis);
...
//                    currentTag0 = null;
                    ...
                    if (output == null) {
                        output = writeField(s1, lastTag0, s2, "<volume>", "<biblScope type=\"vol\">", addSpace, 0);
                    } else {
                        buffer.append(output);
                        lastTag = s1;
                        continue;
                    ...
                    if (output == null) {
                        output = writeField(s1, lastTag0, s2, "<pages>", "<biblScope type=\"pp\">", addSpace, 0);
                    } else {
                        buffer.append(output);
                        lastTag = s1;
                        continue;
                    ...
                    if (output == null) {
                        output = writeField(s1, lastTag0, s2, "<issue>", "<biblScope type=\"issue\">", addSpace, 0);
                    } else {
                        buffer.append(output);
                        lastTag = s1;
                        continue;
...
//			e.printStackTrace();
    ...
    final static private Pattern m3 = Pattern.compile("((^|\\n)( )*\\d{1,3}\\.)");
