...
public class MonographParser extends AbstractParser {
    ...
    private void addTagClose(StringBuilder buffer, String currentTag) {
        ...
        if (currentTag.equals("<cover>")) {
            ...
            buffer.append("</other>\n\n");
        } else if (currentTag.equals("<dummy>")) {
            buffer.append("</dummy>\n\n");
        }
    }
     ...
     * @param id        id
     */
    public Document createTrainingFromPDFWithTEILabel(File inputFile,
                                                         String pathRaw,
                                                         String pathTEI,
                                                         int id) {
        DocumentSource documentSource = null;
        ...
        try {
            // test the dummy model, it's hidden if it's not used
            tagger = TaggerFactory.getTagger(GrobidModels.DUMMY);
            ...
            if (blocks == null) {
                ...
                // gather the features by lines
                String rawText = getAllLinesFeatured(doc);
                String result = null, labelledResult = null;
                if (isNotEmpty(trim(rawText))) {
                    // if the feature data are not empty, give them the labels
                    if (isNotBlank(rawText)) {
                        labelledResult = tagger.label(rawText);
                        //String labelledResult = label(rawText);
                        // extract the labeled data
                        result = resultExtraction(labelledResult, doc, config);
                    }
                }
                ...
                // write the TEI file
                writer = new OutputStreamWriter(new FileOutputStream(outputTEIFile, false), "UTF-8");
                // only for testing dummy model
                writer.write(labelledResult);
                //writer.write(result);
                writer.close();
