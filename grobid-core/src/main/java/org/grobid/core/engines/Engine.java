...
/**
 * Class for managing the extraction of bibliographical
 * informations from PDF documents or raw text.
 *
...
public class Engine implements Closeable {
    ...
    public List<Person> processAuthorsHeader(String authorSequence) throws Exception {
        ...
        return authorParser.processingHeader(inputs);
    }
    ...
    public List<Person> processAuthorsCitation(String authorSequence) throws Exception {
        ...
        return authorParser.processingCitation(inputs);
    }

    /**
     * Parse a list of independent sequences of authors from citations.
     *
     * @param authorSequences - the list of strings corresponding each to a raw sequence of names.
     * @return the list of all recognized structured author objects for each sequence of authors.
     */
    public List<List<Person>> processAuthorsCitationLists(List<String> authorSequences)
            throws Exception {
        return null;
    }
     ...
     */
    public List<Affiliation> processAffiliation(String addressBlock) {
     ...
     *
     * @param addressBlocks - the list of strings corresponding each to a raw affiliation+address.
     * @return the list of all recognized structured affiliation objects for each sequence of affiliation
     *         + address block.
     */
    public List<List<Affiliation>> processAffiliations(List<String> addressBlocks)
            throws Exception {
        if (affiliationAddressParser == null) {
            affiliationAddressParser = new AffiliationAddressParser();
        }
     ...
     */
    public List<org.grobid.core.data.Date> processDate(String dateBlock) {
        if (dateParser == null) {
            dateParser = new DateParser();
        }
        return dateParser.processing(dateBlock);
    }
     ...
     * @return the list of all structured date objects recognized in the string for each inputed
     *         string.
     */
     ...
     * @param consolidate - the consolidation option allows GROBID to exploit
     *                    Crossref web services for improving header information
     * @return the recognized bibliographical object
     */
    public BiblioItem processRawReference(String reference,
                                          boolean consolidate) {
        if (reference != null) {
     ...
     */
    public List<BiblioItem> processRawReferences(List<String> references,
                                                 boolean consolidate)
            throws Exception {
        if (references == null)
            return null;
    ...
    /**
     * Return an object representing the bibliographical information of the header
     * of the current document. The extraction and parsing of the header of the
     * document must have been done to get an instanciated object.
     *
     * @return BiblioItem representing the cibliographical information of the header of the
     *         current document.
     */
     ...
     */
    public List<BibDataSet> processReferences(String inputFile,
                                              boolean consolidate) throws Exception {
        if (citationParser == null) {
            citationParser = new CitationParser();
        }
        List<org.grobid.core.data.BibDataSet> bits =
                citationParser.processingReferenceSection(inputFile, consolidate);
        return bits;
    }
     ...
     * @param name    name of the file to save the downloaded PDF
     */
     ...
     * If null, any languages will be processed
     *
    ...
    public Language runLanguageId(String ext) {
        try {
            // we just skip the 50 first lines and get the next approx. 5000 first characters,
...
//            int nbLine = 0;
     ...
     */
    public String processHeader(String inputFile,
                                boolean consolidate,
                                BiblioItem result) throws Exception {
    ...
    public String processHeader(String inputFile,
                                boolean consolidate,
                                int startPage, int endPage, BiblioItem result) throws Exception {
        if (headerParser == null) {
            headerParser = new HeaderParser();
        }
        // normally the BiblioItem reference must not be null, but if it is the case, we still continue
        // with a new instance, so that the resulting TEI string is still delivered
        if (result == null) {
    ...
    /**
     * Create training data for the header model based on the application of the current
     * header model on a new PDF
     *
     * @param inputFile:  the path of the PDF file to be processed
     * @param pathHeader: the path where to put the header with layout features
     * @param pathTEI:    the path where to put the annotated TEI representation
     *                    (the file to be corrected for gold-level training data)
     * @param id:         an optional ID to be used in the TEI file and the header file
     */
    public void createTrainingHeader(String inputFile,
                                     String pathHeader,
                                     String pathTEI,
                                     int id) {
        if (headerParser == null) {
    ...
    /**
     * Create training data for the full text model based on the application of the current
     * full text model on a new PDF
     *
     * @param inputFile:    the path of the PDF file to be processed
     * @param pathFullText: the path where to put the full text with layout features
     * @param pathTEI:      the path where to put the annotated TEI representation
     *                      (the file to be corrected for gold-level training data)
     * @param id:           an optional ID to be used in the TEI file and the full text file
     */
    ...
    /**
     * Parse and convert the current article into TEI, this method performs
     * the whole parsing and conversion process.
     * If onlyHeader is true, tean only the tei header data will be created.
     *
     * @param inputFile            - absolute path to the pdf to be processed
     * @param consolidateHeader    - the consolidation option allows GROBID to exploit
     *                             Crossref web services for improving header information
     * @param consolidateCitations - the consolidation option allows GROBID to exploit
     *                             Crossref web services for improving citations information
     */
     ...
     * If onlyHeader is true, than only the tei header data will be created.
     *
     * @param inputFile            - absolute path to the pdf to be processed
     * @param consolidateHeader    - the consolidation option allows GROBID to exploit
     *                             Crossref web services for improving header information
     * @param consolidateCitations - the consolidation option allows GROBID to exploit
     *                             Crossref web services for improving citations information
     * @param method - if method is 0, a rule-based is used for the full text part, otherwise
     *                             a machine learning approach is used.
     */
    public String fullTextToTEI(String inputFile,
                                boolean consolidateHeader,
    ...
    /**
     * Process all the PDF in a given directory with a header extraction and produce the
     * corresponding training data format files for manual correction. The goal of this
     * method is to help to produce additional traning data based on an existing model.
     *
     * @param directoryPath - the path to the directory containing PDF to be processed.
     * @param resultPath    - the path to the directory where the results as XML files shall be
     *                      written.
     * @param ind           - identifier integer to be included in the resulting files to identify
     *                      the training case. This is optional: no identifier will be included if ind = -1
     * @return the number of processed files.
     */
    public int batchCreateTrainingHeader(String directoryPath,
                                         String resultPath,
                                         int ind) {
    ...
    /**
     * Process all the PDF in a given directory with a fulltext extraction and produce the
     * corresponding training data format files for manual correction. The goal of this
     * method is to help to produce additional traning data based on an existing model.
     *
     * @param directoryPath - the path to the directory containing PDF to be processed.
     ...
     * @return the number of processed files.
     */
    public int batchCreateTrainingFulltext(String directoryPath,
                                           String resultPath,
                                           int ind) {
        return batchCreateTraining(directoryPath, resultPath, ind, 1);
    }

    private int batchCreateTraining(String directoryPath,
                                    String resultPath,
                                    int ind,
                                    int type) {
        try {
            ...
            for (; n < refFiles.length; n++) {
                ...
                /*else if (type == 2) {
                        createTrainingCitations(pdfFile.getPath(), resultPath, resultPath, ind+n);
                    }*/
            }

            return refFiles.length;
        } catch (Exception e) {
            throw new GrobidException("An exception occured while running Grobid batch.", e);
        }
    }

    /**
     * Extract the headers for all PDF files in a given directory and produce the results
     * as an XML file TEI conformant.
     *
     * @param directoryPath - the path to the directory containing PDF to be processed.
     * @param resultPath    - the path to the directory where the results as XML files shall be
     *                      written.
     ...
     * @return the number of processed files.
     */
    public int batchProcessHeader(String directoryPath,
                                  String resultPath,
                                  boolean consolidate) throws Exception {
    ...
    /**
     * Extract the fulltext for all PDF files in a given directory and produce the results
     * as an XML file TEI conformant.
     *
     * @param directoryPath        - the path to the directory containing PDF to be processed.
     * @param resultPath           - the path to the directory where the results as XML files shall be
     ...
     * @return the number of processed files.
     */
    public int batchProcessFulltext(String directoryPath,
                                    String resultPath,
                                    boolean consolidateHeader,
                                    boolean consolidateCitations) {
    ...
    /**
     * @param directoryPath        input path, folder where the pdf files are supposed to be located
     ...
     * @return
     */
    private int batchProcess(String directoryPath,
                             String resultPath,
                             boolean consolidateHeader,
                             boolean consolidateCitations,
                             int type) {
            ...
            for (; n < refFiles.length; n++) {
                ...
                if (!pdfFile.exists()) {
                    throw new GrobidResourceException("A problem occurs in reading pdf file '" +
                            pdfFile.getAbsolutePath() + "'. The file does not exists. ");
                }
                if (type == 0) {
                    //BiblioItem res = processHeader(pdfFile.getPath(), consolidateHeader);
                    ...
                    if (tei != null) {
                        String outPath = resultPath + "/" +
                                pdfFile.getName().replace(".pdf", GrobidProperties.FILE_ENDING_TEI_FULLTEXT);
                ...
                /*else if (type == 2) {
                        processCitations(pdfFile.getPath(), resultPath, resultPath);
                    }*/
            }
    ...
    /**
     * Get the TEI XML string corresponding to the recognized raw text citation with pointers
     */
    ...
    /**
     * Extract and parse patent references within a patent. Result are provided as PatentItem
     * containing both "WISIWIG" results (the patent reference attributes as they appear in the
     * text) and the attributes in DOCDB format (format according to WIPO and ISO standards).
     * Offset positions are given in the PatentItem object.
     *
     * @param text - the string corresponding to the text body of the patent.
     * @return the list of extracted and parserd patent references as PatentItem object.
     */
    ...
    /**
     * Extract and parse non patent references within a patent. Result are provided as
     * a BibDataSet with offset position instanciated relative to input text.
     *
     * @param text                 - the string corresponding to the text body of the patent.
     * @param consolidateCitations - the consolidation option allows GROBID to exploit
     *                             Crossref web services for improving citations information
     * @return the list of extracted and parserd non patent references as BiblioItem object.
     */
    public List<BibDataSet> processNPLCitationsInPatent(String text,
                                                        boolean consolidateCitations) throws Exception {
        if (referenceExtractor == null) {
            referenceExtractor = new ReferenceExtractor();
        }
    ...
    /**
     * Extract and parse both patent and non patent references within a patent text. Result are
     * provided as a BibDataSet with offset position instanciated relative to input text and
     * as PatentItem containing both "WISIWIG" results (the patent reference attributes as they
     * appear in the text) and the attributes in DOCDB format (format according to WIPO and
     * ISO standards). Patent references' offset positions are also given in the PatentItem object.
     *
     * @param text                 - the string corresponding to the text body of the patent.
     ...
     * @param patentResults        - the list of extracted and parsed patent references as PatentItem object. This
     *                             list must be instanciated before calling the method for receiving the results.
     * @param consolidateCitations - the consolidation option allows GROBID to exploit
     *                             Crossref web services for improving citations information
     */
    public void processAllCitationsInPatent(String text,
        ...
        // we initialize the attribute individually for readability...
        boolean filterDuplicate = false;
        referenceExtractor.extractAllReferencesString(text, filterDuplicate, consolidateCitations, patentResults,
                nplResults);
    }

    /**
     * Extract and parse both patent and non patent references within a patent in ST.36 format. Result are
     * provided as a BibDataSet with offset position instanciated relative to input text and
     * as PatentItem containing both "WISIWIG" results (the patent reference attributes as they
     * appear in the text) and the attributes in DOCDB format (format according to WIPO and
     * ISO standards). Patent references' offset positions are also given in the PatentItem object.
     ...
     * @throws Exception if sth. went wrong
     */
    public void processAllCitationsInXMLPatent(String xmlPath,
                                               List<BibDataSet> nplResults,
                                               List<PatentItem> patentResults,
                                               boolean consolidateCitations) throws Exception {
        if ((nplResults == null) && (patentResults == null)) {
        ...
        // we initialize the attribute individually for readability...
        boolean filterDuplicate = false;
        referenceExtractor.extractAllReferencesXMLFile(xmlPath, filterDuplicate, consolidateCitations,
                patentResults,
                nplResults);
    }

    /**
     * Process an XML patent document with a patent citation extraction and produce the
     * corresponding training data format files for manual correction. The goal of this
     * method is to help to produce additional traning data based on an existing model.
     *
     ...
     *                   written.
     */
    public void createTrainingPatentCitations(String pathXML,
    ...
    /**
     * Process all the XML patent documents in a given directory with a patent citation
     * extraction and produce the corresponding training data format files for manual correction.
     * The goal of this method is to help to produce additional traning data based on an existing
     * model.
     *
     * @param directoryPath - the path to the directory containing XML files to be processed.
     * @param resultPath    - the path to the directory where the results as XML files shall be
     *                      written.
     * @return the number of processed files.
     */
    public int batchCreateTrainingPatentcitations(String directoryPath,
                                                  String resultPath) throws Exception {
        try {
            File path = new File(directoryPath);
            // we process all pdf files in the directory
    ...
    /**
     * Return all textual content without requiring a segmentation.
     * Ignore the toIgnore1 th blocks (default is 0)
     * and the blocks after toIgnore2 th (included, default is -1)
     */
