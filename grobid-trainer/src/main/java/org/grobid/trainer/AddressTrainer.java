...
import org.grobid.trainer.sax.TEIAddressSaxHandler;
...
public class AddressTrainer extends AbstractTrainer {
	...
	/**
	 * Add the selected features to the address model training
	 * 
	 * @param corpusDir
	...
	public int createCRFPPData(final File corpusDir, 
		...
		try {
			...
			for (; n < refFiles.length; n++) {
				...
				final TEIAddressSaxHandler parser = new TEIAddressSaxHandler();
				...
				for(int i=0; i<allTokens.size(); i++) {
					...
					final String names = FeaturesVectorAddress.addFeaturesAddress(allTokens.get(i), 
						allLabeled.get(i), placePositions, countryPositions);
