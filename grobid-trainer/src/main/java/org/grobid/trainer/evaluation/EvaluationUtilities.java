...
public class EvaluationUtilities {
    ...
    public static String taggerRun(ArrayList<String> ress, Tagger tagger)  {
        ...
        // we have to re-inject the pre-tags because they are removed by the JNI parse method
...
//            int cumulated_all = 0;
            ...
            int totalValidFields = 0; // in case a class should not be considered in the training set
            ...
            while (i < labels.size()) {
                ...
                report.append(label).append(" (token): " + "true positives: ").append(tp).
                        append("\n\t\tfalse positives:").append(fp).append("\n\t\ttrue negatives: ").
...
//                    cumulated_all += all;
            ...
            double accuracy = (double) (cumulated_tp + cumulated_tn) / (cumulated_tp + cumulated_fp +
                    cumulated_tn + cumulated_fn);
            report.append("\t").append(TextUtilities.formatTwoDecimals(accuracy * 100));
            ...
            while (stt.hasMoreTokens()) {
                ...
                // the two last tokens, separated by a tabulation, gives the expected label and, last,
                // the resulting label
                StringTokenizer st = new StringTokenizer(line, "\t");
                String currentToken = null;
                String previousToken = null;
