...
public class AcknowledgmentTrainer extends AbstractTrainer {
    ...
    public int createCRFPPData(File corpusPath, File outputFile) {
        return addFeaturesAcknowledgment(corpusPath.getAbsolutePath() + "/tei",
            corpusPath.getAbsolutePath() + "/raw",
            outputFile, null, 1.0);
    }

    /**
     * Add the selected features for the acknowledgment model
     *
    ...
    public int createCRFPPData(final File corpusDir,
                               ...
                               double splitRatio) {
        return addFeaturesAcknowledgment(corpusDir.getAbsolutePath() + "/tei",
     ...
     */
    public int addFeaturesAcknowledgment(String sourceTEIPathLabel,
                ...
                try {
                        ...
                        for (int pp = q; pp < labeled.size(); pp++) {
                            ...
                            if (st.hasMoreTokens()) {
                                ...
                                if (localToken.equals(token)) {
                                    ...
                                    acknowledgment.append(line).append(" ").append(tag);
                                    previousTag = tag;
                                    q = pp + 1;
                                    nbInvalid = 0;
                                    //pp = q + 10;
                            ...
                            if (pp - q > 5) {
                                //LOGGER.warn(name + " / Acknowledgment trainer: TEI and raw file unsynchronized at raw line " + l + " : " + localLine);
                                nbInvalid++;
                                // let's reuse the latest tag
                                if (previousTag != null)
                                    acknowledgment.append(line).append(" ").append(previousTag);
                                break;
                            }
                        }
                        if (nbInvalid > 20) {
                    ...
                    if (nbInvalid < 10) {
                        if ((writer2 == null) && (writer3 != null))
                            writer3.write(acknowledgment.toString() + "\n");
                        if ((writer2 != null) && (writer3 == null))
                            writer2.write(acknowledgment.toString() + "\n");
                        else {
    ...
    public static void main(String[] args) throws Exception {
        ...
        AbstractTrainer.runTraining(new AcknowledgmentTrainer());
        AbstractTrainer.runEvaluation(new AcknowledgmentTrainer());
        System.exit(0);
    }
}
