...
/**
 * Tests the RESTful service of the grobid-service project. This class can also tests a remote system, when 
 * setting system property org.grobid.service.test.uri to host to test.
 * 
...
//		return("http://localhost:8080/grobid-service-0.0.3-SNAPSHOT/grobid");
	 ...
	 * If system property is given the port given as property will be returned.
	 * @return
	 * @throws IOException
	 */
		...
		try {
			if (server== null)
			{
				...
				if (System.getProperty(PROP_TEST_HOST)!= null)
				{
					...
					this.setHost(host);
					if (host==null)
						logger.warn("Cannot read value of system property '"+PROP_TEST_HOST+"', because it is null.");
				}
				...
				if (host==null)
				{
					...
					host= LOCALHOST+":"+port+"/";
					this.setHost(host);
					logger.debug("started grobid-service for test on: "+ this.getHost());
	...
	public void testFullyRestLessHeaderDocument() throws Exception {
		...
		logger.debug("calling "+ this.getHost()+GrobidPathes.PATH_GROBID+"/"+GrobidPathes.PATH_HEADER);
		
		service = Client.create().resource(this.getHost()+GrobidPathes.PATH_GROBID+"/"+GrobidPathes.PATH_HEADER);
		response = service.type(MediaType.MULTIPART_FORM_DATA).accept(MediaType.APPLICATION_XML)
	...
	public void testFullyRestLessFulltextDocument() throws Exception {
		...
		FormDataMultiPart form = new FormDataMultiPart();
		form.field("fileContent", pdfFile, MediaType.MULTIPART_FORM_DATA_TYPE);
		logger.debug("calling "+ this.getHost()+GrobidPathes.PATH_GROBID+"/"+GrobidPathes.PATH_FULL_TEXT);
		
		service = Client.create().resource(this.getHost()+GrobidPathes.PATH_GROBID+"/"+GrobidPathes.PATH_FULL_TEXT);
		response = service.type(MediaType.MULTIPART_FORM_DATA).accept(MediaType.APPLICATION_XML)
						.post(ClientResponse.class, form);
		assertEquals(Status.OK.getStatusCode(), response.getStatus());
	...
	public void testRestDate() throws Exception {
		...
		service = Client.create().resource(this.getHost()+GrobidPathes.PATH_GROBID+"/"+GrobidPathes.PATH_DATE);
	...
	public void testRestNamesHeader() throws Exception {
		...
		service = Client.create().resource(this.getHost()+GrobidPathes.PATH_GROBID+"/"+GrobidPathes.PATH_HEADER_NAMES);
		response = service.post(ClientResponse.class, formData);
	...
	public void testRestNamesCitations() throws Exception {
		...
		service = Client.create().resource(this.getHost()+GrobidPathes.PATH_GROBID+"/"+GrobidPathes.PATH_CITE_NAMES);
		response = service.post(ClientResponse.class, formData);
	...
	public void testRestAffiliations() throws Exception {
		...
		service = Client.create().resource(this.getHost()+GrobidPathes.PATH_GROBID+"/"+GrobidPathes.PATH_AFFILIATION);
		response = service.post(ClientResponse.class, formData);
...
//			if (!propFile.exists()) {
//				throw new GrobidException("Cannot run tests for grobid service, because the property file for" +
...
//		else fail("cannot find the host for the grobidService, please check configuration file: "+ propFileName);
...
//		if (tmpDir== null)
//			throw new GrobidException("Cannot start test, because tmp folder is not set.");
